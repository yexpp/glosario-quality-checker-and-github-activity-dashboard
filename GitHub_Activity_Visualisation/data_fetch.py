...
def get_single_commit_data(commit, retries=3):
    """
    Retrieve detailed information about a single commit with retries on failure.
    ...
    for attempt in range(retries):
        try:
            ...
            time.sleep(DEFAULT_SLEEP + attempt * DEFAULT_SLEEP)
            ...
            if e.status == 403:  # Rate limit exceeded
                print(f"[!] Rate limit hit on {commit.sha}")
                time.sleep(RETRY_SLEEP_BASE + attempt * RETRY_SLEEP_BASE)
            else:
                print(f"[!] GitHub error on commit {commit.sha}: {e}")
        except Exception as e:
            print(f"[!] Unexpected error on commit {commit.sha}: {e}")
...
def get_commits(max_count=None):
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            results = list(executor.map(get_single_commit_data, commits))
        return filter_bots(pd.DataFrame(results))
    except Exception as e:
...
def get_commit_comments(max_count=None):
    """
    Fetch comments made on commits, limited by max_count.
    Returns filtered DataFrame excluding bots.
    """
    try:
        comments = repo.get_comments()
        data = extract_comments_from_iterable(comments, comment_type="commit_comment", max_count=max_count)
        return filter_bots(pd.DataFrame(data))
    except Exception as e:
...
def get_pull_request_comments(max_count=None):
    """
    ...
    for pr in pulls:
        if max_count and count >= max_count:
            break
        count = fetch_and_append_pr_comments(pr, "get_issue_comments", "pr_issue_comment", data, count, max_count)
        if max_count and count >= max_count:
            break
        count = fetch_and_append_pr_comments(pr, "get_review_comments", "pr_review_comment", data, count, max_count)
    return filter_bots(pd.DataFrame(data))
...
def get_readme_contributors_remote_df():
    ...
    for c in contributors:
        ...
        if not login:
            continue
        rows.append({"login": login.strip().lower(), **{t: 1 for t in contribs}})
    if not rows:
        return pd.DataFrame()
